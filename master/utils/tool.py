...
def adjust_cosine_learning_rate(optimizer, epoch, lr_max=5e-4, lr_min=5e-7, reset_epoch=10):
    curr_epoch = epoch % reset_epoch
    new_lr = lr_min + 0.5 * (lr_max - lr_min) * (1 + math.cos(math.pi * curr_epoch / reset_epoch))
    for param_group in optimizer.param_groups:
